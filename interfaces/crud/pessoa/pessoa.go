...
func Update(person *entity.Pessoa) error {
	erro := db.ExecutaComando("update pessoa set nome = '" + person.Nome + "', cargo = '" + person.Cargo + 
	"', orgao = '" + person.Orgao + "', remuneracaodomes = " + strconv.FormatFloat(person.Remuneracaodomes) + 
	"', redutorsalarial = " + strconv.FormatFloat(person.RedutorSalarial) + 
