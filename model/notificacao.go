...
//Insert new pessoa
func Insert(user *entity.Notificacao) error {
	erro := db.ExecutaComando("insert into notificacao (idarquivotransparencia, nome, cargo, orgao, remuneracaodomes, redutorsalarial, totalliquido, update, clientedobanco) values(" + strconv.FormatInt(user.Cpf, 10) + ",'" + user.Nome + "', '" + user.Senha + "','" + user.Email + "'," + strconv.FormatBool(user.FuncionarioPuplico) + ");")
	return erro
}
